...
Cada cita nueva incluye por defecto una alarma sonora 5 minutos antes y admite hasta dos alarmas independientes. La usuaria puede editar o eliminar la inicial y parametrizar la cantidad y la unidad (minutos, horas o días) de ambas. Las dos se calculan siempre antes de la hora de inicio; no representan alertas posteriores. Android usa `@capacitor/local-notifications`, el sonido propio `appointment_alarm.ogg`, canal de importancia máxima, vibración, alarmas exactas y acciones Detener/Posponer. Requiere aceptar notificaciones en Android 13+ y permitir alarmas exactas en Android 12+ cuando el sistema lo solicite. Android no permite que una app ignore el volumen, No molestar o la configuración manual del canal del dispositivo. En web se usa Notification API más alarma audible en primer plano; un navegador no garantiza sonido ni ejecución con la PWA completamente cerrada. Sin backend push no existe entrega web fiable con la app cerrada.

La alerta `Tiempo previsto cumplido` es independiente: solo aplica a una cita cuyo servicio ya fue iniciado y se reproduce cuando el tiempo transcurrido alcanza la duración prevista. `Posponer 5 min` es una acción manual sobre una alarma ya disparada y puede reaparecer después de la hora de inicio.

La disponibilidad se valida autoritativamente en `appointmentService` dentro de las transacciones `rw` de creación y edición. Dos intervalos `[dateTime, dateTime + duration)` no pueden cruzarse; compartir únicamente el límite final/inicial sí está permitido. La edición excluye su propio id y solo revalida al cambiar fecha/hora o duración, para no bloquear correcciones no temporales de citas legacy ya solapadas.
